...
begin
  ...
  try
    ...
    M.Lines.Text :=
      ...
      '  • Запись аудио: системный звук (WASAPI loopback) и/или' + LineEnding +
      '    микрофон (WASAPI capture, с фоллбэком на DirectShow).' + LineEnding +
      '  • Автодетект тишины (VAD) — паузы вырезаются после Stop' + LineEnding +
      '    отдельным проходом silenceremove с откалиброванным под' + LineEnding +
      '    каждый микрофон порогом.' + LineEnding +
      '  • Список записей с переименованием и удалением — кнопка ▾' + LineEnding +
      '    справа внизу. При переименовании автоматически обновляются' + LineEnding +
      '    ссылки во всех журналах заданий.' + LineEnding +
      '  • ffmpeg.exe встроен в EXE — приложение распространяется' + LineEnding +
      '    одним файлом.' + LineEnding +
      LineEnding +
      ...
      '   4 — REC: старт/стоп записи. Файл попадает в audio/.' + LineEnding +
      '   5 — Автодетект тишины (VAD).' + LineEnding +
      '   6 — статус записи: «запись MM:SS» или «H:MM:SS».' + LineEnding +
      ...
      '  • Источники: системный звук (loopback устройства' + LineEnding +
      '    воспроизведения по умолчанию) и/или микрофон.' + LineEnding +
      '  • Микрофон захватывается через WASAPI capture (быстрый' + LineEnding +
      '    старт ~150 мс). Если WASAPI не нашёл устройство — фоллбэк' + LineEnding +
      '    на DirectShow.' + LineEnding +
      '  • Файлы сохраняются в audio/YYYYMMDD_HHMMSS.mp3.' + LineEnding +
      '    Папка настраивается в меню «Запись аудио → Папка для' + LineEnding +
      '    аудио…». Качество MP3 (64/128/256 kbps) — там же.' + LineEnding +
      '  • Автодетект тишины (VAD): порог калибруется по микрофону' + LineEnding +
      '    при первой записи и кэшируется в config.xml. Сбросить —' + LineEnding +
      '    «Сбросить калибровку микрофонов». Чувствительность можно' + LineEnding +
      '    подвинуть на ±8 dB (Высокая/Средняя/Низкая).' + LineEnding +
      '  • Тишина вырезается после Stop отдельным проходом' + LineEnding +
      '    silenceremove (это занимает 1–3 сек на типичной записи).' + LineEnding +
      LineEnding +
      'Список записей' + LineEnding +
      '  • Кнопка ▾ справа внизу открывает таблицу с записанными' + LineEnding +
      '    mp3-файлами; сортировка по дате убывания.' + LineEnding +
      '  • Колонки: ▶ (воспроизвести в системном плеере), имя,' + LineEnding +
      '    длительность, размер, дата записи, задача (если файл' + LineEnding +
      '    привязан к задаче).' + LineEnding +
      '  • F2 на строке — переименование. При сохранении файл' + LineEnding +
      '    переименовывается на диске, и во всех журналах дней' + LineEnding +
      '    обновляются ссылки на этот файл.' + LineEnding +
      '  • Правый клик → «Удалить файл…» — удаление с подтверждением;' + LineEnding +
      '    ссылки в журналах также убираются.' + LineEnding +
      LineEnding +
      'Отладка' + LineEnding +
      '  • «Запись аудио → Полное логирование (debug)» включает' + LineEnding +
      '    подробный вывод ffmpeg в bin/ffmpeg_audio.log.' + LineEnding +
      '  • «Открыть лог записи» — открывает файл в системном' + LineEnding +
      '    редакторе. Удобно для отправки разработчику.' + LineEnding +
      LineEnding +
